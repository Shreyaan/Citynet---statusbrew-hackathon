    ...
    return (
        ...
        >
            ...
            <div className="flex items-center space-x-2">
                ...
                {isHovered && <span className="text-white font-bold">User Dashboard</span>}
            </div>
            ...
            <nav className="flex flex-col space-y-4">
                ...
                >
                    <SidebarLink icon={<FiCalendar />} label="Events" isHovered={isHovered} />
                    {/* Sub-links for Create Event and View Events */}
                    {isEventsHovered && (
                        <div className={`flex flex-col ml-10 space-y-2 ${isHovered ? 'visible' : 'hidden'} transition-all duration-300`}>
...
const SidebarSubLink = ({ label, href }: { label: string; href: string }) => {
    return (
        <a href={href} className="text-gray-400 text-sm pl-2 hover:text-white cursor-pointer transition duration-300">
            {label}
        </a>
    );
};
