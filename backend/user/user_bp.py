...
def edit_user():
    ...
    try:
        ...
        if not user:
            return jsonify({"error": "User not found"}), 404  # Return 404 if user is not found
        ...
        return jsonify({"error": f"Failed to update user information: {str(e)}"}), 500  # Return a 500 error
...
def get_user_profile():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve user profile: {str(e)}"}), 500  # Return a 500 error
